...
for i in range(m):
    for j in range(n + 1):
        for k in range(t):
            M.addConstr(gb.quicksum(z[i, j, l, k]
                                    for l in range(n + 1)) == y[i, j, k])
            M.addConstr(gb.quicksum(z[i, l, j, k]
                                    for l in range(n + 1)) == y[i, j, k])
            for l in range(n + 1):
                if j == l:
                    if j != 0 and l != 0:
...
for i in range(m):
    for k in range(t):
        for j in range(n):
            for l in range(n):
                M.addConstr(s[i, j + 1, k] + (sd[j + 1] + grid[j + 1, l]) *
                            z[i, j + 1, l, k] <= s[i, l, k] + bigM * (1 - z[i, j + 1, l, k]))
...
for i in range(m):
    for j in range(n):
        for k in range(t):
            M.addConstr(s[i, j + 1, k] + bigM *
                        (1 - y[i, j + 1, k]) >= et[j + 1])
            M.addConstr(s[i, j + 1, k] + sd[j + 1] - bigM *
                        (1 - y[i, j + 1, k]) <= lt[j + 1])
M.update()
