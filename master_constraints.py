...
def masterConstraints(M, d, x, y, m, n, t, q, q2, q3, Q, nN, f, gb):
    '''constraint 2'''
    for j in range(n):
        M.addConstr(gb.quicksum(x[i,j] for i in range(m)) == nN[j+1] * d[j])
    M.update()
    '''constraint 2'''
    for j in range(n):
        M.addConstr(gb.quicksum(gb.quicksum( y[i, j + 1, k] for k in range(t)) for i in range(m)) == nN[j+1] * f[j+1] * d[j])
    M.update()
    '''constraint 3'''
    for i in range(m):
        for j in range(n):
