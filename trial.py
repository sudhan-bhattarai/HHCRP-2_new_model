...
'''print solution'''
solution_d, solution_x, solution_y, solution_z, solution_s = M.getAttr('x', d), M.getAttr('x', x), M.getAttr('x', y), M.getAttr('x', z), M.getAttr('x',s)
var_d, var_x, var_y, var_z, var_s = np.empty([n]), np.empty([m,n]), np.empty([m,n+1,t]), np.empty([m,n+1,n+1,t]), np.empty([m,n+1,t])
for i in range(m):
    for j in range(n):
